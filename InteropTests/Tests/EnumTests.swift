...
class EnumTests: WinRTTestCase {
    ...
    func testFlagsBitwiseOperators() throws {
        XCTAssertEqual(~FlagsEnum.all, try Enums.bitwiseAnd(FlagsEnum.all))
        ...
        XCTAssertEqual(FlagsEnum.bit0 ^ FlagsEnum.bit16, try Enums.bitwiseAnd(FlagsEnum.bit0 ^ FlagsEnum.bit16))
    }
}
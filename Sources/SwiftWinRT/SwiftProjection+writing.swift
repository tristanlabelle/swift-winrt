    ...
    func writeModule(_ module: Module, outputDirectoryPath: String) {
        for (namespace, types) in module.typesByNamespace {
            let sourceFileWriter = SwiftSourceFileWriter(
                output: FileTextOutputStream(path: "\(outputDirectoryPath)\\\(namespace).swift"))
    ...
    fileprivate func writeTypeDefinition(_ typeDefinition: TypeDefinition, to writer: some SwiftTypeDeclarationWriter) {
        ...
        if let classDefinition = typeDefinition as? ClassDefinition {
            ...
            writer.writeStruct(
                ...
                writeFields(of: typeDefinition, to: writer, defaultInit: true)
            }
        }
    ...
    fileprivate func writeFields(of typeDefinition: TypeDefinition, to writer: SwiftRecordBodyWriter, defaultInit: Bool) {
        ...
        for field in typeDefinition.fields.filter({ $0.visibility == .public }) {
            ...
            writer.writeStoredProperty(
                ...
                name: Casing.pascalToCamel(field.name),
                type: type,
                initializer: defaultInit ? getDefaultValue(type) : nil)
        }
    }

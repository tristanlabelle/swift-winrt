...
internal func writeInterfaceDefinition(
    ...
    if SupportModules.WinRT.getBuiltInTypeKind(interface) != nil {
        ...
        if swiftBug72724 == false { writer.writeImport(exported: true, kind: .typealias, module: SupportModules.WinRT.moduleName, symbolName: typeName) }
        if swiftBug72724 == nil { writer.output.writeFullLine("#else", groupWithNext: true) }
        if swiftBug72724 == true { writer.writeImport(exported: true, kind: .protocol, module: SupportModules.WinRT.moduleName, symbolName: typeName) }

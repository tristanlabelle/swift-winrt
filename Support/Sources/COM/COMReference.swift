...
public struct COMReference<Interface>: ~Copyable /* where Interface: COMIUnknownStruct */ {
    ...
    public func queryInterface(_ id: COMInterfaceID) throws -> IUnknownReference /* where Other: COMIUnknownStruct */ {
        try interop.queryInterface(id)
    }

    public func queryInterface<Other>(_ id: COMInterfaceID, type: Other.Type = Other.self) throws -> COMReference<Other> /* where Other: COMIUnknownStruct */ {
        try interop.queryInterface(id, type: Other.self)
    }

    // Should require COMIUnknownStruct but we run into compiler bugs.
    public consuming func reinterpret<Other>(to type: Other.Type = Other.self) -> COMReference<Other> /* where Other: COMIUnknownStruct */ {
        let pointer = self.pointer
        discard self

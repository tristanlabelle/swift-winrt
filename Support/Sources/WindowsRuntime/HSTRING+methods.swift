...
extension CWinRTCore.ABI_HString {
    public static func create(_ value: String) throws -> CWinRTCore.ABI_HString? {
        ...
        return try chars.withUnsafeBufferPointer {
            ...
            try HResult.throwIfFailed(CWinRTCore.ABI_WindowsCreateString($0.baseAddress!, UInt32(chars.count), &result))
            return result
        }
    }

    public static func delete(_ value: CWinRTCore.ABI_HString?) {
        let hr = CWinRTCore.ABI_WindowsDeleteString(value)
...
extension Optional where Wrapped == CWinRTCore.ABI_HString {
    public func duplicate() throws -> Self {
        var result: CWinRTCore.ABI_HString?
        try HResult.throwIfFailed(CWinRTCore.ABI_WindowsDuplicateString(self, &result))
        return result
    }

    public func toString() -> String {
        var length: UInt32 = 0
        guard let ptr = CWinRTCore.ABI_WindowsGetStringRawBuffer(self, &length) else { return "" }
